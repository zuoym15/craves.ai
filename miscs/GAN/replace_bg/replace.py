...
class background_replace(object):
    ...
    def replace(self, img, color = 'green'):
        ...
        bg = cv2.resize(bg, (s*W, s*H)) 
        ul = [np.random.randint((s/2-1)*H, (s/2)*H), np.random.randint((s/2-1)*W, (s/2)*W)]#upperleft corner
        img[mask] = bg[ul[0]:ul[0]+H, ul[1]:ul[1]+W, :][mask]

        return img

def main():
    source_dir = 'C:/Users/Yiming/Desktop/arm-pose/data/real_20181010'
    target_dir = 'C:/Users/Yiming/Desktop/arm-pose/data/real_20181113'

    bg_replacer = background_replace('C:/Users/Yiming/Documents/test2017')

    L, F = read_jpg(source_dir)

    cnt = 0

    for i in np.random.choice(len(L), 1000, replace = False):
        img_path = L[i]
        replaced_img = bg_replacer.replace(cv2.imread(img_path), 'white')
        cv2.imwrite(os.path.join(target_dir, F[i]), replaced_img)
        cnt += 1
        if cnt % 10 == 0:
            print(cnt)
